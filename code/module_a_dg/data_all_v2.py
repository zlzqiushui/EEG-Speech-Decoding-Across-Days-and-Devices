"""
data_all_v2.py — 公共通道版本

与 data_all_v1.py 的差异:
  - 不再根据 eeg_device 切换通道策略。
  - 始终使用 day1/day2/brk 三设备的公共通道 (COMMON_CHANNELS, 共 57 个)。
  - EEG 数据按 COMMON_CHANNELS 顺序对齐，in_dim 固定为 57。
  - eeg_device 仅用于区分数据目录/故事范围，不影响通道选择。
"""
...
# =========================================================================
# 三设备公共通道 (57 个)
# day1/day2 (Neurascan) 和 brk (Neuracle) 的交集，
# 排除了 neurascan 独有的 CPz, P1, P2, I1, I2, HEO, VEO
# 以及 brk 独有的 AF7, AF8, ECG, HEOL, HEOR, VEOL, VEOU。
# 按标准 10-20 系统顺序排列。
# =========================================================================
COMMON_CHANNELS = [
    'Fp1', 'Fpz', 'Fp2', 'AF3', 'AF4',
...
def _align_to_common_channels(eeg, current_ch_names, recording_path):
    ...
    Args:
        eeg: (time, channels) 或 (channels, time) 的 numpy array
        current_ch_names: 当前录音的通道名列表
        recording_path: 录音文件路径 (仅用于报错)

    Returns:
        eeg: (time, num_common_channels) 的 numpy array
    """
    # 确保 eeg 是 (time, channels)
    ...
    for ch in COMMON_CHANNELS:
        if ch in current_ch_names:
            ...
            raise ValueError(
                ...
                f"Available channels: {current_ch_names}"
            )
...
def get_EEG_emb_feat_story_range(sub_ls, feat_dict, feat_frames_id_dict, feat_keys,
    ...
    v2 关键变化:
      - 无论 eeg_device 是什么，始终提取 COMMON_CHANNELS (57 个) 并按此顺序排列。
      - eeg_device 仅影响故事范围选择，不影响通道维度。
      - in_dim 固定为 57。
    """
...
def get_EEG_emb_feat(sub_ls, feat_dict, feat_frames_id_dict, feat_keys,
                     seg_len=5, fs=64, std_eeg='std', phase='train',
                     feature_name='envelope'):
    """加载 EEG 数据 (全脑版本, 按公共通道对齐, v2)。"""
    num_channels = NUM_COMMON_CHANNELS
    seg_len_emb = int(fs * seg_len)
    EEG_dict = {}
    EEG_feat_index_dict = {}
    ...
    for subject_path in tqdm(all_subjects):
        ...
        for recording in all_recordings:
            ...
            eeg = eeg.astype(np.float32)
            # v2: 始终按 COMMON_CHANNELS 对齐
            # 此格式的数据没有 ch_names, 假设前 NUM_COMMON_CHANNELS 即为公共通道
            ...
            if len(eeg) > stimulus_feat_len:
                ...
                eeg = np.concatenate([eeg, np.zeros((stimulus_feat_len - len(eeg), num_channels))], axis=0)
            eeg_frames = eeg.reshape(stimulus_feat.shape[0], seg_len_emb, -1)
            EEG_sub.append(eeg_frames)
            EEG_feat_index_sub.append(feat_frames_id_dict[stimulus_filename])
            stimulus_index_sub.append(feat_keys.index(stimulus_filename) * np.ones(len(eeg_frames), dtype=np.int64))
...
def get_EEG_emb_feat_test(sub_ls, feat_dict, feat_frames_id_dict, feat_keys,
                          train_stimulus_filename, seg_len=5, fs=64, std_eeg='std',
                          feature_name='envelope'):
    """加载测试 EEG 数据 (全脑版本, 按公共通道对齐, v2)。"""
    num_channels = NUM_COMMON_CHANNELS
    seg_len_emb = int(fs * seg_len)
    EEG_dict = {}
    EEG_feat_index_dict = {}
